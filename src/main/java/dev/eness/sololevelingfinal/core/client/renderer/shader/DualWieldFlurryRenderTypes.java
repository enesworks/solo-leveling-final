...
public class DualWieldFlurryRenderTypes extends RenderStateShard {
   ...
   public static void registerShaders(RegisterShadersEvent event) throws IOException {
      event.registerShader(
         new ShaderInstance(
            event.getResourceProvider(), new ResourceLocation("sololeveling", "rendertype_dual_wield_flurry"), WorldShaderVertexFormat.NEW_ENTITY
         ),
         shader -> flurryShader = shader
      );
   }
   ...
   public static RenderType flurry(ResourceLocation texture) {
      ...
      CompositeState state = CompositeState.builder()
         .setShaderState(new ShaderStateShard(() -> flurryShader))
      ...
      return RenderType.create("dual_wield_flurry", WorldShaderVertexFormat.NEW_ENTITY, Mode.QUADS, 512, false, true, state);
   }
}
