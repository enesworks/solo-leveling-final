...
public final class GlacialPursuitRenderTypes extends RenderStateShard {
   ...
   public static void registerShaders(RegisterShadersEvent event) throws IOException {
      event.registerShader(
         new ShaderInstance(event.getResourceProvider(), new ResourceLocation("sololeveling", "rendertype_glacial_pursuit"), WorldShaderVertexFormat.NEW_ENTITY),
         loaded -> shader = loaded
      );
   }

   public static RenderType effect(ResourceLocation fallback) {
      return shader == null ? RenderType.entityTranslucentEmissive(fallback) : EFFECT_CACHE.computeIfAbsent(fallback, GlacialPursuitRenderTypes::createEffect);
   }
   ...
   private static RenderType createEffect(ResourceLocation fallback) {
      ...
      return RenderType.create("glacial_pursuit", WorldShaderVertexFormat.NEW_ENTITY, Mode.QUADS, 512, false, true, state);
   }
}
