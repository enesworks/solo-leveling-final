...
public class SwordBeamProjectileRenderTypes extends RenderStateShard {
   ...
   public static void registerShaders(RegisterShadersEvent event) throws IOException {
      event.registerShader(
         new ShaderInstance(
            event.getResourceProvider(), new ResourceLocation("sololeveling", "rendertype_sword_beam_projectile"), WorldShaderVertexFormat.NEW_ENTITY
         ),
         shader -> projectileShader = shader
      );
   }
   ...
   public static RenderType projectile(ResourceLocation texture) {
      ...
      CompositeState state = CompositeState.builder()
         .setShaderState(new ShaderStateShard(() -> projectileShader))
      ...
      return RenderType.create("sword_beam_projectile", WorldShaderVertexFormat.NEW_ENTITY, Mode.QUADS, 512, false, true, state);
   }
}
