   ...
   public static void registerShaders(RegisterShadersEvent event) {
      try {
         event.registerShader(
            new ShaderInstance(
               event.getResourceProvider(), new ResourceLocation("sololeveling", "rendertype_hunter_evaluation_background"), DefaultVertexFormat.POSITION_TEX
            ),
            loaded -> shader = loaded
         );
      } catch (Exception exception) {
         shader = null;
         SololevelingMod.LOGGER.warn("Failed to load Hunter Evaluation shader; using Java fallback.", exception);
      }
   }
