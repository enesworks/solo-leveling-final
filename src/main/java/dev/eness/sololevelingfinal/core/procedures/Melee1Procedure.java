      ...
      if (entity != null) {
         ...
         if (entity.isAlive()) {
            ...
            if (attack_duration == 1.0) {
               ...
               direction = new Vec3(entity.getLookAngle().x, 0.0, entity.getLookAngle().z).normalize();
            }

            if (attack_duration == 14.0) {
               entity.setDeltaMovement(entity.getDeltaMovement().add(direction.scale(1.0)));
               if (distance <= 6.0) {
                  target.hurt(
