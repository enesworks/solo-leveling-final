            ...
            if (entity.getPersistentData().getString("state").equals("spin")) {
                  ...
                  for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(5.0), e -> true)
                     ...
                     if (entityiterator instanceof LivingEntity living && ShadowMonarchManager.canShadowDamage(entity, living)) {
                        ...
                        living.hurt(
                           ...
                           ShadowIgrisCombatBalance.abilityDamage(entity, 10.0)
                        );
                        if (world instanceof ServerLevel _level) {
                           _level.sendParticles(
                              ParticleTypes.SWEEP_ATTACK, living.getX(), living.getY() + entity.getBbHeight() / 2.0F, living.getZ(), 3, 0.1, 0.1, 0.1, 0.0
                           );
                        }
                     }
                  }
               ...
               if (entity.getPersistentData().getDouble("MF") == 37.0) {
                  ...
                  for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10.0), e -> true)
                     ...
                     .sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center)))
                     .toList()) {
                     if (entityiterator instanceof LivingEntity living && living.onGround() && ShadowMonarchManager.canShadowDamage(entity, living)) {
                        living.hurt(
                           new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK), entity),
                           ShadowIgrisCombatBalance.abilityDamage(entity, 13.5)
                        );
                        if (world instanceof ServerLevel _level) {
                           _level.sendParticles(
                              ParticleTypes.EXPLOSION, living.getX(), living.getY() + entity.getBbHeight() / 2.0F, living.getZ(), 1, 0.1, 0.1, 0.1, 0.0
                           );
                        }
                     }
                  }
