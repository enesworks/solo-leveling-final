...
public class ShadowOrcHurtProcedure {
   public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
      if (entity != null && sourceentity != null) {
         double rand = 0.0;
         rand = Math.random();
         if (!(sourceentity instanceof Player _plr && _plr.getAbilities().instabuild) && rand >= 0.81) {
               ...
               () -> {
                  ...
                  for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10.0), e -> true)
                     ...
                     if (entity != entityiterator
                        && (entityiterator instanceof TamableAnimal _tamEnt ? _tamEnt.getOwner() : null)
                           != (entity instanceof TamableAnimal _tamEnt ? _tamEnt.getOwner() : null)
                        && (entity instanceof TamableAnimal _tamEnt ? _tamEnt.getOwner() : null) != entityiterator
                        && entity.isAlive()) {
                        entityiterator.hurt(
                           new DamageSource(
