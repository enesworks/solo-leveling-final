...
public final class RakanGameTests {
    ...
    public static void vfxMeshesAreFiniteAndBounded(GameTestHelper test) {
        ...
        for (var style : dev.eness.sololeveling3.vfx.RakanVfxMesh.Style.values()) {
            var frames = new java.util.ArrayList<Object>();
            int duration = dev.eness.sololeveling3.vfx.RakanVfxMesh.duration(style);
            for (int age=0; age<duration; age++) {
                var quads = new java.util.ArrayList<double[]>();
                dev.eness.sololeveling3.vfx.RakanVfxMesh.emit(style,age,Vec3.ZERO,new Vec3(0,0,1),(a,b,c,d,r,g,blue,alpha) -> {
